...
In the **Data Visualisation with R** short course, we learned how to present what we **observe** in data. Probability helps us understand what we should **expect** and underpins statistical testing. Some of the ideas in this section may feel abstract at first. Their purpose will become clearer when we apply them in statistical testing in later chapters.
...
What is the mean? `r fitb(45)`

What is the standard deviation? `r fitb(3.2)`

What are the values that are 1, 2, and 3 standard deviations **above** the mean, in that order? Write your answers separated by commas (for example: `1, 2, 3`). `r fitb("48.2, 51.4, 54.6", ignore_case = TRUE, ignore_ws = TRUE, regex=TRUE)`

What are the values that are 1, 2, and 3 standard deviations **below** the mean, in that order? Write your answers separated by commas (for example: `1, 2, 3`). `r fitb("41.8, 38.6, 35.4", ignore_case = TRUE, ignore_ws = TRUE, regex=TRUE)`

Which probability expression are we modelling? `r mcq(c("P(X<3.2)", answer = "P(X>50)", "P(Z>50)", "P(Z<50)"))`
...
![](images/clipboard-214006668.png)

`r hide("Solution")`

We are looking for $P(X>50)$, where $X \sim (45, 3.2^2)$.

![](images/clipboard-1790724682.png)
