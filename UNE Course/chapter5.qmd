...
[![Photo by Dan Gold on Unsplash](images/clipboard-1693076206.png){width="576"}](https://unsplash.com/photos/macro-shot-of-vegetable-lot-4_jhDO54BYg?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText)
...
You may see an error appears when we try to convert the data.

![](images/Screenshot%202025-12-02%20232723.png)

This is telling us there are duplicates for the ID beyond the two that we are expecting. Let's try using the code provided in the warning to see what's going on.
...
Because we are keeping the dataset simple, a natural research question is whether there is a significant difference in participant BMI before and after the living well program.
...
We can check for skewness and normality. What plots can we use to check these conditions?

`r hide("I need a hint")`

We can check for skewness with a boxplot or histogram.

For normality, we can refer to a Q-Q plot.
...
#| warning: false
#| message: false
# Create a box plot with the ggplot2 package for our new `diff` variable
# use `mistyrose` to fill the box, and `pink4` for the border
...
#| warning: false
#| message: false
# Create a Q-Q plot with the ggplot2 package for our new `diff` variable
# use `navyblue` to add colour to the points, and `steelblue` to add colour to the line
# Don't forget to label your x-axis "Theoretical quantiles" and y-axis "Sample quantiles"
### write your code here
```
...
10. What is the degrees of freedom? `r fitb("548", ignore_case = TRUE, ignore_ws = TRUE)`
11. What is the test statistic? `r fitb("9.0896", ignore_case = TRUE, ignore_ws = TRUE)`
12. What is the p-value (written exactly as shown in the output)? `r fitb("2.2e-16", ignore_case = TRUE, ignore_ws = TRUE)`
13. Should we accept or reject the null hypothesis? Write "accept" or "reject". `r fitb("reject", ignore_case = TRUE, ignore_ws = TRUE)`

We can also present the paired t-test results in a clean gtsummary table. Because the measurements are paired, we first need to convert the data from wide format to long format so that baseline and week 8 are stored in a single measurement column, with an ID linking each pair of observations.

```{r}
paired_data <- living_well_wide %>%
  mutate(id = row_number()) %>%
  pivot_longer(
    cols = c(Baseline, Post_week8),
    names_to = "Time",
    values_to = "Score"
  ) %>%
  
  # Make sure Baseline is compared against Post_week8
  mutate(
    Time = factor(Time, levels = c("Baseline", "Post_week8"))
  ) %>%
...
paired_data %>%
  ...
  # Show mean and SD at Baseline and Week 8
  tbl_summary(
    by = Time,
    include = Score,
  ...
  # Add the paired t-test p-value
  add_p(
    test = Score ~ "paired.t.test",
    group = id,
    test.args = Score ~ list(alternative = "greater")
  )
```


The p-value ($t_{548}=9.09, p=2.2*10^{-16}$) is much less than the 5% significance level (p\<0.05). We used `greater` because our hypothesis was that BMI would go down after the program.

The t-test calculates the difference as baseline minus post week 8, so if BMI really decreased, that difference would be positive.

Using `greater` tells R to test whether the average of those differences is greater than zero, meaning baseline values were higher than post week 8 values.
...
With 95% confidence, there is a mean BMI decrease between 0.22 and 0.34 in participants after the program compared to when participants started the program. Because this interval includes zero, the data indicates that there is a significant difference before and after the program. **Note that if 0 was [IN]{.underline} in the confidence interval, this would suggest there is [not]{.underline} a significant difference**.

A nicer way we can present our results is with `gtsummary`. 
...
Take some time to think of research questions you might ask comparing three or more groups. For this section, we will explore the research question: *does education level impacts post-program BMI?*
...
living_well_aov <- iiving_well_clean %>%
  _____(
...
Let's be sure to remove any \`missing values. How can we do this?
...
10. What is the degrees of freedom? `r fitb("6", ignore_case = TRUE, ignore_ws = TRUE)`
11. What is the test statistic? `r fitb("2.5519", ignore_case = TRUE, ignore_ws = TRUE)`
12. What is the p-value (written exactly as shown in the output)? `r fitb("0.01915", ignore_case = TRUE, ignore_ws = TRUE)`
13. Should we accept or reject the null hypothesis? Write "accept" or "reject". `r fitb("reject", ignore_case = TRUE, ignore_ws = TRUE)`
...
Let's present the data in a nicer format using the `gtsummary` package.
```{r, message=FALSE, warning=FALSE}
library(gtsummary)

living_well_aov %>%
  
  # Summarise Post_week8 for each education group
  tbl_summary(
    by = Education,
    include = Post_week8,
    
    # Show mean and standard deviation for each group
    statistic = all_continuous() ~ "{mean} ({sd})"
  ) %>%
  
  # Add the p-value from a one-way ANOVA
  # var.equal = TRUE assumes equal variance between groups
  add_p(
    test = Post_week8 ~ "oneway.test",
    test.args = Post_week8 ~ list(var.equal = TRUE)
  )
```
