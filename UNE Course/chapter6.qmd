...
[![Photo by Fitsum Admasu on Unsplash](images/clipboard-1233813349.png){width="555"}](https://unsplash.com/photos/silhouette-of-three-women-running-on-grey-concrete-road-oGv9xIl7DkY?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText)
...
8.  What order should our labels be in? Please list the labels separated by commas (for example: `x, y, z` You can refer to the labels above. `r fitb("Normal weight, Overweight, Obese", ignore_case = TRUE, ignore_ws = TRUE)`
...
11. What is the test statistic? `r fitb("9.4985", ignore_case = TRUE, ignore_ws = TRUE)`
...
[![Photo by Towfiqu barbhuiya on Unsplash](images/clipboard-1749607671.png){width="635"}](https://unsplash.com/photos/a-person-stacking-coins-on-top-of-a-table-jpqyfK7GB4w?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText)
...
We can also use `gtsummary` to present chi-squared tests. Here, tbl_cross() creates a contingency table showing the relationship between annual income and gender, and add_p() adds the chi-squared test p-value.
...
ecec_chi %>%
  ...
  # Create a contingency table of Annual Income by Gender
  tbl_cross(
    row = Annual_Income,
    col = Gender,
  ...
  # Add the p-value from a chi-squared test
  add_p(
    test = "chisq.test"
  )
```
