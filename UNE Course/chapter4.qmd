...
[![Photo by niko nguyen on Unsplash](images/clipboard-2607643104.png){width="540"}](https://unsplash.com/photos/a-red-train-traveling-down-train-tracks-next-to-tall-buildings-L7U0tY7nhMA?utm_source=unsplash&utm_medium=referral&utm_content=creditCopyText)
...
**NOTE: At this stage, we will drop missing values from the dataset. If you are interested in understanding missing values and when it is appropriate to drop them, you can refer to the *Data Visualisation in R* short course. It is also recommended that you refer to** [Missing Data Imputation in R: Missing data R tutorial](https://libguides.princeton.edu/R-Missingdata) **by Princeton University on handling missing data.**
...
```{r}
one_sided_t_test <- t.test(ecec_2$Work_Commute, mu = 29, alternative = "less")
one_sided_t_test
```
...
```{r}
two_sided_t_test <- t.test(ecec_2$Work_Commute, mu = 29, alternative = "two.sided")
two_sided_t_test
```
...
### `gt` package
`
A nicer way we present this information is with `gt`. 
...
data.frame(
  ...
  Result = c(
    round(one_sided_t_test$estimate, 2),
    round(one_sided_t_test$statistic, 2),
    round(one_sided_t_test$parameter, 0),
    round(one_sided_t_test$p.value, 3),
    paste0(
      round(two_sided_t_test$conf.int[1], 2),
      " to ",
      round(two_sided_t_test$conf.int[2], 2)
    )
  )
) %>%
  gt() %>%
...
# Comparing Groups with `gtsummary`

`gtsummary` is a useful package for creating clean statistical summary tables. It can summarise variables by group and add p-values from appropriate statistical tests, making it easy to present descriptive statistics and group comparisons together.

Let's take a look at an example with our two sample t test.
...
ecec_new1 %>%
  tbl_summary(
    by = Gender,
    include = Commute_Time,
    statistic = all_continuous() ~ "{mean} ({sd})"
  ) %>%
  add_p(
    test = Commute_Time ~ "t.test",
    test.args = Commute_Time ~ list(var.equal = TRUE)
  )
```

The p-value tests whether there is evidence that mean work commute time differs between males and females. This is the same comparison we made earlier using `t.test()`, but `gtsummary` presents the group summaries and p-value together in a cleaner table.
